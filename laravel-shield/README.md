...
```bash
composer require yourname/laravel-shield --dev
```
...
```bash
composer global require yourname/laravel-shield
...
*Replace the vendor name `yourname` and GitHub URL before publishing to Packagist; tag `v1.0.0` for the first release.*
